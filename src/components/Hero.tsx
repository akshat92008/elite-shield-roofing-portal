    ...
    try {
      const response = await fetch('http://localhost:3001/api/leads', {
        ...
        body: JSON.stringify({
          ...
          phoneNumber: phone,
          estimatedCost: 15000 + Math.random() * 10000, // Simulated logic
          monthlyPayment: 199 + Math.random() * 100, // Simulated logic
          tenantId: 'ROOF_001',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          ...
          <motion.div
            ...
            className="max-w-2xl"
          >
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-charcoal/10 shadow-sm mb-6">
              <span className="flex h-2 w-2 rounded-full bg-safety-orange"></span>
              <span className="text-xs font-semibold uppercase tracking-wider text-charcoal/80">Next-Gen Roofing Solutions</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
              <div className="flex items-center gap-2">
                <div className="flex -space-x-2">
                  {[1, 2, 3, 4].map((i) => (
                    <img key={i} src={`https://picsum.photos/seed/user${i}/100/100`} alt="User" className="w-10 h-10 rounded-full border-2 border-slate-grey object-cover" />
                  ))}
                </div>
                <div className="ml-2">
